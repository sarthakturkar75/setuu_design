...
import { X, Search, UserPlus, Filter, Edit2 } from 'lucide-react';
...
export function ResourcePoolModal({ projectId, onClose, onRefresh }: any) {
  ...
  const handleAssign = async (userId: string) => {
    ...
    if (res.success) {
      ...
      onRefresh();
      onClose(); // Optional: close or let them assign more
    } else {
  ...
  const renderTable = (personnel: any[], title: string, subtitle: string) => (
    <div className="mb-8">
      <div className="mb-3">
         ...
         <p className="text-xs text-on-surface-variant">{subtitle} ({personnel.length} found)</p>
      </div>
      {personnel.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-lg border border-outline-variant/50 bg-surface">
              ...
              {personnel.map(p => (
                <tr key={p.id} className="hover:bg-surface-variant/10">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-on-surface-variant font-mono mt-0.5 text-ellipsis overflow-hidden max-w-[200px]">{p.id}</div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-right flex justify-end gap-2">
                    ...
                    <button 
                      ...
                      className="px-3 py-1.5 bg-primary text-on-primary rounded text-xs font-semibold hover:bg-primary/90 disabled:opacity-50 inline-flex items-center gap-1"
                    >
                      <UserPlus className="w-3 h-3" />
                      {submittingId === p.id ? 'Assigning...' : 'Assign'}
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <div className="p-6 border-b border-outline-variant/30 bg-surface-variant/20">
          <form onSubmit={handleSearch} className="flex gap-3 max-w-md">
            <div className="relative flex-1">
              ...
              <input 
                type="text" 
                placeholder="Search by skill (e.g. 'Certified Welder')"
                value={searchSkill}
            ...
            <button type="submit" className="px-4 py-2 bg-primary text-on-primary rounded-lg text-sm font-semibold hover:bg-primary/90">
              Filter
            </button>
            {searchSkill && (
              <button type="button" onClick={() => { setSearchSkill(""); fetchResources(); }} className="px-4 py-2 bg-surface border border-outline-variant text-on-surface rounded-lg text-sm hover:bg-surface-variant">
                Clear
              </button>
            )}
          </form>
