...
export type FileScanState = 'idle' | 'scanning' | 'clean' | 'infected'
...
export function FileUpload({ onFileSelect, accept, multiple, scanState = 'idle', className, ...props }: FileUploadProps) {
  ...
  return (
      <div
        className={cn(
          "relative flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer transition-colors bg-surface-container-lowest",
          isDragging 
            ? "border-primary bg-primary/5" 
            : "hover:bg-surface-container-low",
          scanState === 'idle' && !isDragging && "border-outline-variant",
          scanState === 'scanning' && "border-semantic-sky bg-semantic-sky/5",
          scanState === 'clean' && "border-semantic-emerald bg-semantic-emerald/5",
          scanState === 'infected' && "border-semantic-crimson bg-semantic-crimson/5",
          className
        )}
