      ...
      <div className="bg-surface w-full max-w-4xl rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-outline-variant/30">
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col lg:flex-row gap-6">
          <div className="flex-1 space-y-6">
            ...
            <section className="bg-surface-variant/20 rounded-lg p-4 border border-outline-variant/30 mt-6">
              <h3 className="text-sm font-semibold text-on-surface flex items-center gap-2 mb-4">
                <MapPin className="w-4 h-4 text-primary" /> Delivery & Geofence Timeline
              </h3>
              {(!material.tracking_timeline || material.tracking_timeline.length === 0) ? (
                <p className="text-xs text-on-surface-variant">No tracking events logged yet.</p>
              ) : (
                <div className="space-y-3 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-outline-variant before:to-transparent">
                  {material.tracking_timeline.map((event: any, i: number) => (
                    <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                      <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white bg-surface-variant shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2">
                        <MapPin className="w-4 h-4 text-primary" />
                      </div>
                      <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-lg border border-outline-variant/50 bg-surface shadow-sm">
                        <div className="flex justify-between items-center mb-1">
                          <h4 className="font-bold text-sm text-on-surface">{event.event}</h4>
                          <span className="text-[10px] text-on-surface-variant font-mono">{new Date(event.timestamp).toLocaleString()}</span>
                        </div>
                        {event.location && (
                          <div className="text-xs text-on-surface-variant font-mono mt-2 bg-surface-variant/20 p-2 rounded">
