...
export async function POST(request: Request) {
  try {
    ...
    const { data: drawing } = await adminSupabase
    ...
    const { data: allDrawings } = await adminSupabase
      .from("drawing_versions")
      ...
      messages: [
        ...
        },
        {
          ...
          content: [
            {
              type: "image_url",
              image_url: { url: drawing.file_url, detail: "low" },
            },
          ],
        },
      ],
    ...
    if (allDrawings) {
      for (const link of foundLinks) {
        const targetDrawing = allDrawings.find(
          (d) => d.drawing_name === link.target_name,
        );
        if (targetDrawing) {
          newLinks.push({
