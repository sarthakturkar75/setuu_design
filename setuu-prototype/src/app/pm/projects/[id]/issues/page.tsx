...
import { getProjectIssues, getRootCauses, getIssueAnalytics } from "@/app/actions/issueActions";
...
export default function IssuesList({ params }: { params: Promise<{ id: string }> }) {
  ...
  const columns = [
    ...
    { key: "title", header: "Issue", cell: (row: any) => <span className="font-semibold text-primary hover:underline cursor-pointer">{row.title}</span> },
    { key: "severity", header: "Severity", cell: (row: any) => (
      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
        row.severity === 'Critical' ? 'bg-semantic-crimson text-white' : 
        row.severity === 'High' ? 'bg-semantic-amber text-black' : 
        'bg-surface-variant text-on-surface-variant'
      }`}>{row.severity}</span>
    )},
    { key: "sla", header: "SLA Deadline", cell: (row: any) => (
       row.status === 'Resolved' ? 
       <span className="text-xs text-semantic-emerald font-semibold">RESOLVED</span> : 
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        actions={
          <button onClick={() => setShowCreateModal(true)} className="flex items-center gap-2 px-4 py-2 bg-primary text-on-primary rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors">
      ...
      {analytics && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           <div className="md:col-span-2 bg-surface-container rounded-xl border border-outline-variant/50 p-4">
             <h3 className="text-sm font-semibold text-on-surface mb-2">Root Cause Clustering</h3>
             <RootCauseDistributionChart data={analytics.rootCauseDistribution} />
           </div>
           <div>
      ...
      {showCreateModal && (
        <CreateIssueModal 
          ...
          onRefresh={() => { setShowCreateModal(false); loadData(); }} 
        />
      )}
      ...
      {qaIssue && (
        <LogQAInspectionModal 
          ...
          onRefresh={() => { setQaIssue(null); loadData(); }} 
        />
      )}
    </div>
  );
