...
export default function GlobalResourcesHub() {
  ...
  const columns = [
    {
      key: "name",
      header: "Resource Group / Individual",
      cell: (row: any) => (
    ...
    },
    {
      ...
      header: "Actual",
      cell: (row: any) => (
        <span className="font-jetbrains text-sm">{row.actual_hours || 0}h</span>
      ),
    },
    {
      key: "status",
    ...
    },
    {
      ...
      cell: () => (
        <div className="flex items-center gap-2">
          <button
            className="p-1.5 hover:bg-surface-variant rounded-md text-on-surface-variant hover:text-primary transition-colors"
            title="View Timesheet"
          >
            <Clock className="w-4 h-4" />
          </button>
          <button
            className="p-1.5 hover:bg-emerald-500/10 rounded-md text-on-surface-variant hover:text-semantic-emerald transition-colors"
            title="Approve Hours"
          >
  ...
  const maxHours = analytics
    ? Math.max(
      10,
      ...analytics.topProjects.map((p: any) =>
        Math.max(p.allocated, p.actual),
      ),
    )
    : 100;
  ...
  return (
    <div className="flex flex-col h-full bg-surface">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3">
            <Link
              href="/pm/resources/analytics"
              className="hidden sm:flex items-center gap-2 px-4 py-2 border border-outline-variant bg-surface text-on-surface rounded-lg text-sm font-semibold hover:bg-surface-variant transition-colors"
            >
      ...
      <div className="flex-1 overflow-y-auto p-6 max-w-[1800px] mx-auto w-full flex flex-col gap-6">
        {loading ? (
          <div className="p-6 text-center animate-pulse text-on-surface-variant">
            Loading analytics...
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="p-6 col-span-1 lg:col-span-2 flex flex-col">
              <h3 className="font-semibold text-on-surface mb-4">
                Top 5 Projects Resource Allocation
              </h3>
              <div className="flex-1 min-h-[200px] flex items-end gap-6 border-b border-l border-outline-variant p-4 relative pt-10">
                <div className="absolute top-2 right-4 flex items-center gap-4 text-xs font-medium text-on-surface-variant">
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className="w-3 h-3 rounded bg-primary" /> Actual
                  </div>
                </div>

                {analytics?.topProjects.map((proj: any, idx: number) => {
                  const allocH = Math.max(
                    10,
                    (proj.allocated / maxHours) * 100,
                  );
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-end gap-1 h-full w-full justify-center">
                        <div
                          className="w-8 md:w-12 bg-outline-variant/30 rounded-t-sm"
                          style={{ height: `${allocH}%` }}
                          title={`${proj.allocated} Allocated`}
                        />
                        <div
                          className={`w-8 md:w-12 rounded-t-sm ${isOver ? "bg-crimson/80" : "bg-primary/80"}`}
                          style={{ height: `${actH}%` }}
                          title={`${proj.actual} Actual`}
                        />
                      </div>
                      <span
                        className="text-xs font-medium text-on-surface-variant whitespace-nowrap truncate max-w-[80px]"
                        title={proj.name}
                      >
                        {proj.name}
                      </span>
                ...
                {analytics?.topProjects.length === 0 && (
                  <div className="w-full text-center text-sm text-on-surface-variant pb-8">
            ...
            <Card className="p-6 flex flex-col justify-center bg-surface-variant/30 border-dashed border-2 border-outline-variant items-center text-center">
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${analytics?.totalVariance > 0 ? "bg-semantic-amber/10" : "bg-semantic-emerald/10"}`}
              >
                <TrendingUp
                  className={`w-8 h-8 ${analytics?.totalVariance > 0 ? "text-semantic-amber" : "text-semantic-emerald"}`}
                />
              </div>
              <h3 className="text-3xl font-bold text-on-surface font-jetbrains">
                {analytics?.totalVariance > 0 ? "+" : ""}
                {analytics?.totalVariance || 0} hrs
              </h3>
              <p className="text-on-surface-variant font-medium mt-1">
                Portfolio Variance{" "}
                {analytics?.totalVariance > 0 ? "(Overrun)" : "(Saved)"}
              </p>
          ...
          >
            ...
            <Select
              options={[
                ...
                ...Array.from(
                  new Set(resources.map((r) => r.project_name).filter(Boolean)),
                ).map((p) => ({ label: p as string, value: p as string })),
              ]}
              value={projectFilter}
              onChange={(val: string) => setProjectFilter(val)}
            />
          </FilterBar>
