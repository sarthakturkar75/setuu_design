...
import {
  ShieldAlertIcon, CheckCircleIcon, XCircleIcon,
  FileTextIcon, QrCodeIcon, BanknoteIcon, PenToolIcon, StarIcon, PlayIcon
} from "lucide-react";
...
export default function ProjectHandoverConsole() {
  ...
  // Enterprise States
  const [gatewayStatus, setGatewayStatus] = useState<{ allowed: boolean, reasons: string[] } | null>(null);
  ...
  const handleInitCloseout = async () => {
    ...
    const res = await createHandover(projectId, { package_name: "Final Project Closeout Dossier" });
    if (res.success) {
  ...
  const handleClientSignOff = async () => {
    ...
    const res = await submitClientSignOff(handover.id, Number(npsInput), urlData.publicUrl);
  ...
  if (loading) return <div className="p-12 text-center animate-pulse text-on-surface-variant">Loading Closeout Console...</div>;
    ...
    <div className="p-6 space-y-6 max-w-[1200px] mx-auto">
      ...
      {!handover ? (
        <Card className="p-12 flex flex-col items-center justify-center text-center border-dashed border-2 border-outline-variant bg-surface-variant/10">
          ...
          <Button variant="primary" onClick={handleInitCloseout} disabled={actionLoading}>
        ...
        <div className="space-y-6">
          <Card className="p-6 flex justify-between items-center bg-surface-variant/20 border-primary/20">
            <div>
              <h2 className="text-lg font-bold text-on-surface">{handover.package_name}</h2>
              <p className="text-sm text-on-surface-variant mt-1">Status dictates which actions are currently unlocked below.</p>
            </div>
            <StatusBadge label={handover.status} tone={handover.status === "Approved" ? "emerald" : handover.status === "Draft" ? "slate" : "amber"} />
          </Card>
                ...
                gatewayStatus.allowed ? (
                  <div className="flex items-center gap-3 text-semantic-emerald p-4 bg-semantic-emerald/10 rounded-lg border border-semantic-emerald/20">
                    ...
                    <div>
                      ...
                      <p className="text-xs mt-0.5">No critical issues or pending VOs detected in the database.</p>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-start gap-3 text-semantic-crimson p-4 bg-semantic-crimson/10 rounded-lg border border-semantic-crimson/20">
                    ...
                    <div>
                      ...
                      <ul className="text-xs space-y-1 list-disc list-inside ml-2">
                        {gatewayStatus.reasons.map((r, i) => <li key={i}>{r}</li>)}
                      </ul>
                    </div>
                  </div>
                )
            ...
            <Card className="p-6 space-y-4">
              ...
              {generatedDocUrl && (
                <div className="p-3 bg-primary/10 border border-primary/20 rounded-lg flex justify-between items-center text-sm">
                  <span className="font-semibold text-primary">PDF Manual Generated</span>
                  <a href={generatedDocUrl} target="_blank" rel="noreferrer" className="text-primary hover:underline font-bold">View PDF</a>
                </div>
              )}
              ...
              <div className="flex flex-col xl:flex-row gap-3">
                <Button
                  ...
                  disabled={actionLoading || !gatewayStatus?.allowed || handover.status !== "Draft"}
              ...
              {generatedQRs.length > 0 && (
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-4 p-4 border border-outline-variant/30 rounded-lg bg-surface-variant/20">
                  {generatedQRs.map(qr => (
                    <div key={qr.assetId} className="flex flex-col items-center gap-2">
                      <img src={qr.qrUrl} alt={`QR`} className="w-16 h-16 rounded shadow-sm border border-outline-variant/50 bg-white p-1" />
                      <span className="text-[10px] text-center font-semibold text-on-surface-variant leading-tight truncate w-full">{qr.name}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <Card className="p-6 space-y-4">
              ...
              {handover.status === "Approved" ? (
                <div className="text-sm text-semantic-emerald font-semibold flex flex-col gap-3 p-4 bg-semantic-emerald/10 border border-semantic-emerald/20 rounded-lg">
                  <div className="flex items-center gap-2"><CheckCircleIcon className="w-5 h-5" /> Client digitally signed.</div>
                  <a href={handover.client_signature_url} target="_blank" className="underline text-xs ml-7 font-bold text-semantic-emerald">View Signature Record</a>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="flex items-center gap-2 px-3 py-2 border border-outline-variant rounded-lg w-full max-w-[200px]">
                    ...
                    <input
                      ...
                      value={npsInput} onChange={e => setNpsInput(e.target.value)}
                  ...
                  <div className="flex flex-col gap-2 border border-dashed border-outline-variant p-4 rounded-lg bg-surface-variant/10">
                    <label className="text-xs font-semibold text-on-surface">Upload Signature Image (Required)</label>
                    <input
                      type="file"
                      accept="image/*"
                  ...
                  <Button
                    variant="primary"
                    className="w-full"
                    disabled={actionLoading || handover.status !== "Pending Signature" || !signatureFile}
            ...
            <Card className="p-6 flex flex-col justify-between">
              <div>
                ...
                <p className="text-sm text-on-surface-variant mb-6">
                  Release the final 5% retention pool back to the vendors. This will fire a webhook directly to your integrated Accounting ERP. Requires Handover to be 100% Approved.
                </p>
              </div>
              <Button
                variant="outline"
