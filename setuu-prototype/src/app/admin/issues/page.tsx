...
export default function IssuesConsolePage() {
  ...
  // New state for resolution action
  ...
  // Action Handler to resolve the issue from the Global Console
  const handleResolveIssue = async () => {
    if (!selectedIssueId) return;
    setResolveLoading(true);

    const res = await markIssueResolved(selectedIssueId);

    if (res.success) {
      toast.success("Issue successfully resolved!");
      // Re-fetch the list locally to update the UI instantly
      const updated = await getIssues();
      setIssues(updated);
    } else {
  ...
  const getSeverityBadge = (severity: string) => {
    switch (severity) {
      case "Critical":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-crimson/10 text-crimson border border-crimson/20">
            <AlertOctagon className="w-3 h-3" /> Critical
          </span>
        );
      case "High":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-semantic-amber/10 text-semantic-amber border border-semantic-amber/20">
            High
          </span>
        );
      case "Medium":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-sky/10 text-sky border border-sky/20">
            Medium
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-surface-variant text-on-surface-variant">
            Low
          </span>
        );
    }
  ...
  const columns = [
      ...
      cell: (row: any) => (
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="text-xs font-jetbrains text-primary">
              {row.display_id || (row.id ? row.id.split("-")[0] : "ISSUE")}
            </span>
        ...
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center shrink-0">
            <span className="text-[10px] font-bold text-primary">
              {row.assignee_name ? row.assignee_name.charAt(0) : "?"}
            </span>
          </div>
    ...
    },
    {
      ...
      sortable: true,
      cell: (row: any) => (
        <span className="font-jetbrains text-sm text-on-surface-variant">
          {row.created_at
  ...
  return (
    <div className="flex flex-col h-full bg-surface">
      <PageHeader
        ...
        actions={
          <Link
            ...
            className="flex items-center gap-2 px-4 py-2 bg-crimson text-white rounded-lg text-sm font-semibold hover:bg-crimson/90 transition-colors shadow-elevation-l1"
          >
      ...
      <div className="flex-1 overflow-hidden flex max-w-[1800px] mx-auto w-full">
        {/* Main List Area */}
        <div
          className={`flex-1 flex flex-col p-6 overflow-y-auto transition-all ${selectedIssueId ? "hidden xl:flex xl:w-2/3" : "w-full"}`}
        >
          <FilterBar onClear={() => { }} onApply={() => { }}>
            <div className="w-full sm:w-64 relative">
              <Search className="w-4 h-4 text-on-surface-variant absolute left-3 top-1/2 -translate-y-1/2" />
              <TextInput
                placeholder="Search issue ID or keywords..."
                className="pl-9"
                value={searchTerm}
          ...
          <div className="w-full xl:w-1/3 border-l border-outline-variant bg-surface flex flex-col shadow-elevation-l3 xl:shadow-none animate-in slide-in-from-right-4 duration-300">
            <div className="p-6 border-b border-outline-variant flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <span className="font-jetbrains text-primary font-bold">
                  {selectedIssue.display_id || "ISSUE"}
                </span>
                <button
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="font-medium text-on-surface font-jetbrains">
            ...
            <div className="p-6 border-t border-outline-variant bg-surface flex flex-col sm:flex-row gap-3">
              <button
                ...
                disabled={resolveLoading || selectedIssue.status === "Resolved"}
                className={`flex-1 px-4 py-2 border rounded-lg text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${selectedIssue.status === "Resolved"
                  ? "border-emerald/20 text-emerald bg-emerald/10 cursor-not-allowed"
                  : "bg-surface text-primary border-outline-variant hover:bg-surface-variant"
                  }`}
              >
