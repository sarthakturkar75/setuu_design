...
import { CreateMilestoneModal } from '@/components/ui/CreateMilestoneModal';
...
export default function MilestoneKanbanPage() {
  ...
  const [selectedDept, setSelectedDept] = React.useState<string | undefined>(undefined);
    ...
    return (
      ...
      >
        ...
        <div className={`flex-1 bg-surface-variant/30 rounded-xl p-3 flex flex-col gap-3 overflow-y-auto ${isOverWIP ? 'border border-semantic-crimson/50' : 'border border-outline-variant/30'}`}>
          ...
          {columnId === "todo" && (
            <button
              onClick={() => { setSelectedDept(dept); setIsAddModalOpen(true); }}
              className="w-full mt-2 py-2 flex items-center justify-center gap-2 text-on-surface-variant hover:bg-surface-variant/50 rounded-lg transition-all text-sm border border-dashed border-outline-variant hover:border-primary/50"
            >
