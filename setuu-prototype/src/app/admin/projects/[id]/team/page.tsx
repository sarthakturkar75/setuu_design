...
export default function ProjectTeamPage() {
  ...
  // Dynamic Internal Categorization based on organization ID matching
  const internals = team.filter(t =>
    t.role !== 'client' &&
    (t.organization_id === organizationId || t.employment_type === 'Internal Employee' || !t.employment_type || t.employment_type === '')
  );

  // If not a client and not internal, they are external
  const vendors = team.filter(t =>
    t.role !== 'client' &&
    !internals.some(internal => internal.id === t.id)
  );
